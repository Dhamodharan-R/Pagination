 ...
 let arr = [
  ...
  /* Creating container div and appending to the body */
  let container = document.createElement('div');
  container.setAttribute("class","container");
  document.body.append(container);

  /* Creating ul for items to be displayed */
  let unorderedList = document.createElement('ul');
  unorderedList.setAttribute("class","unorderedList");

  /* Creating div for pagination */
  let paginator = document.createElement('div');
  paginator.setAttribute("class","paginator");

  /* Appending ul and paginator to container div */
  container.append(unorderedList,paginator);
  ...
  function buildPage(currentPage){
    ...
    let slisedArray =(arr.slice(trimStart,trimEnd));
    

    slisedArray.forEach((x)=>{
        let list = document.createElement('li');
        list.setAttribute("class","list");
        list.innerHTML= `Id : ${x.id}<br/> Name : ${x.name}<br/> Email :${x.email}`;
...
/* Function for buiding page numbers  */
function pagination(){
    for(i=1;i<=numberOfPages;i++){
        let btn = document.createElement('div');
        btn.setAttribute("class","btn");
        btn.innerHTML=`<button value="${i}" onclick="changePage(${i})">${i}</button>`;
...
function changePage(pageno){
    ...
    display.innerText =`Current Page no : ${currentPage}`;
